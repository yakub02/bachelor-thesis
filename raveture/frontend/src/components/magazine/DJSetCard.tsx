...
export function DJSetCard({ set }: { set: DJSet }) {
  return (
    <div className="bg-surface border border-border hover:border-accent transition-colors duration-200 group">
      <div className="relative w-full h-40 bg-bg-dark flex items-center justify-center overflow-hidden">
        {set.thumbnail_url ? (
          ...
          <Music2 className="text-accent/30 w-16 h-16" />
        )}
        ...
        {set.genres.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {set.genres.slice(0, 3).map(g => (
              <span key={g} className="text-accent text-xs border border-accent/30 px-1.5 py-0.5 uppercase tracking-wider">
                {g}
              </span>
            ))}
          </div>
