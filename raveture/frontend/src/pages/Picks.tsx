  ...
  return (
    <div className="relative min-h-screen bg-bg-dark text-white">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-8 pt-32 pb-16">
        {/* Header */}
        <div className="mb-12">
          <span className="bg-accent text-black text-xs font-black px-2 py-1 uppercase tracking-widest">
            RT PICKS
          </span>
          <h1 className="text-5xl md:text-7xl font-black uppercase tracking-tighter text-white mt-4">
            RAVETURE
            <br />
            <span className="text-accent">PICKS</span>
          </h1>
          <p className="text-text-muted text-sm mt-4 max-w-lg">
            Kurátorský výběr událostí, které stojí za to. Každý pick je ručně vybraný týmem RAVETURE.
          </p>
        </div>
        ...
        {!loading && picks.length === 0 && (
          <div className="text-center py-24">
            <span className="text-accent text-6xl block mb-4">◆</span>
            <p className="text-text-muted uppercase tracking-widest text-sm">
              Žádné picks momentálně
            </p>
          </div>
        )}
        ...
        {!loading && picks.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {picks.map((pick) => (
              <div key={pick.id} className="pick-card bg-surface border border-border hover:border-accent transition-colors duration-200">
                <Link to={`/events/${pick.event.slug}`}>
                  {pick.event.cover_image_url ? (
                    ...
                    <div className="w-full h-52 bg-bg-dark flex items-center justify-center">
                      <span className="text-accent text-6xl font-black">◆</span>
                    </div>
                  )}
                </Link>
                <div className="p-6">
                  <span className="text-accent text-xs font-black uppercase tracking-widest">◆ RT PICK</span>
                  <Link to={`/events/${pick.event.slug}`}>
                    <h2 className="text-white font-black uppercase text-xl mt-1 hover:text-accent transition-colors">
                      {pick.event.name}
                    </h2>
                  </Link>
                  <p className="text-text-muted text-sm mt-1">
                    {new Date(pick.event.starts_at).toLocaleDateString('cs-CZ', {
                      weekday: 'long',
                      day: 'numeric',
                      month: 'long',
                      year: 'numeric',
                  ...
                  <p className="text-white text-sm mt-4 leading-relaxed border-l-2 border-accent pl-4">
                    {pick.reason}
                  </p>
